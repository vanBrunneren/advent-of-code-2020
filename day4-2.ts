...
function calculateHeightWithUnit(unit: string, height: number) : boolean {
    if(unit == "cm") return isBetweenNumbers(height, 150, 193);
    else return isBetweenNumbers(height, 59, 76);
}

function calculateHeight(heightInput: string) : boolean {
    let unit = heightInput.substr(-2, 2);
    let height = Number(heightInput.substring(0, heightInput.length -2));
    return calculateHeightWithUnit(unit, height);
}

let conditionObject = {
    "byr": birthYear => birthYear.length === 4 && isBetweenNumbers(birthYear, 1920, 2002), 
...
function checkCondition(keyValue: string[]) {
    if(conditionObject[keyValue[0]]) {
        return conditionObject[keyValue[0]](keyValue[1]);
    }
    return true;
}

function countOccasionsInLine(line: string) : boolean {
    return line
        .split(" ")
        .map(pair => pair.split(":"))
        .map(checkCondition)
